...
function assessFit(jobDescription: string) {
  ...
  const summaries: Record<string, string> = {
    strong: `Based on this analysis, I'd recommend moving forward. There's strong alignment across ${matched.length} core skill areas including ${matched
      .slice(0, 3)
      .map((m) => categoryLabels[m])
      .join(
        ', '
      )}. The gaps noted below are areas I'm aware of and can address — they shouldn't be blockers.`,
              ...
              {result && verdictStyle && (
                ...
                >
                  <div className="p-5 space-y-5">
                    {/* Verdict badge */}
                    <div
                      className={`manga-panel ${verdictStyle.bg} ${verdictStyle.border} border px-4 py-2 inline-block`}
                    >
                      <span
                        className={`font-mono text-lg font-black tracking-wider ${verdictStyle.color}`}
                      >
                        {verdictStyle.label}
                      </span>
                    </div>
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="space-y-2">
                              {matched.slice(0, 3).map((key) => (
                                <div
                                  key={key}
                                  className="manga-panel bg-green-500/5 border border-green-500/20 px-4 py-3"
                                >
                                  <p className="font-mono text-xs font-bold text-green-500 mb-1">
                                    ✓ {categoryLabels[key]}
                                  </p>
                                  <p className="text-sm text-muted-foreground leading-relaxed">
                                    {categoryMatchStatements[key]}
                                  </p>
                                </div>
                              ))}
                            </div>
                        ...
                        {unmatched.length > 0 && (
                          <div className="space-y-2">
                            ...
                            <div className="manga-panel bg-amber-500/5 border border-amber-500/20 px-4 py-3">
                              <p className="text-sm text-muted-foreground leading-relaxed">
                                No direct keyword matches for{' '}
                                <span className="text-amber-500 font-medium">
                                  {unmatched.map((key) => categoryLabels[key]).join(', ')}
                                </span>
                                . These aren't dealbreakers — adjacent experience and fast ramp-up
                                ability cover the gap.
                              </p>
                            </div>
                          </div>
                        )}
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="space-y-2">
                              {unmatched.slice(0, 3).map((key) => (
                                <div
                                  key={key}
                                  className="manga-panel bg-red-400/5 border border-red-400/20 px-4 py-3"
                                >
                                  <p className="font-mono text-xs font-bold text-red-400 mb-1">
                                    ✗ {categoryLabels[key]}
                                  </p>
                                  <p className="text-sm text-muted-foreground leading-relaxed">
                                    This role requires expertise here that falls outside current
                                    core competencies.
                                  </p>
                                </div>
                              ))}
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="space-y-2">
                              {matched.map((key) => (
                                <div
                                  key={key}
                                  className="manga-panel bg-accent/5 border border-accent/20 px-4 py-3"
                                >
                                  <p className="font-mono text-xs font-bold text-accent mb-1">
                                    ↝ {categoryLabels[key]}
                                  </p>
                                  <p className="text-sm text-muted-foreground leading-relaxed">
                                    {categoryTransferStatements[key]}
                                  </p>
                                </div>
                              ))}
                            </div>
