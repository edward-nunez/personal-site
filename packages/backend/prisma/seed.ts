...
async function main() {
  ...
  const experience1 = await prisma.experience.upsert({
    ...
    create: {
      ...
      endDate: null,
      description: 'Leading cloud infrastructure and security initiatives across multiple product teams.',
      achievements: [
  ...
  const project1 = await prisma.project.upsert({
    where: { slug: 'cloud-orchestrator' },
    update: {},
    create: {
      title: 'Cloud Orchestrator',
      slug: 'cloud-orchestrator',
      description: 'A Kubernetes-native deployment orchestration platform that automates multi-cloud infrastructure provisioning with built-in security scanning and compliance checks.',
  ...
  const project2 = await prisma.project.upsert({
    where: { slug: 'godot-space-explorer' },
    update: {},
    create: {
      title: 'Space Explorer',
      slug: 'godot-space-explorer',
      description: 'A lightweight browser-based space exploration game built with Godot Engine. Features procedural generation, physics-based movement, and a relaxing ambient soundtrack.',
  ...
  const project3 = await prisma.project.upsert({
    where: { slug: 'personal-site-v2' },
    update: {},
    create: {
      title: 'Personal Site v2',
      slug: 'personal-site-v2',
      description: 'This portfolio site — a full-stack application built with React, Express, PostgreSQL, and deployed on DigitalOcean Kubernetes with CI/CD.',
      shortDescription: 'Full-stack portfolio with admin panel and blog.',
      technologies: ['React', 'TypeScript', 'Express', 'PostgreSQL', 'Prisma', 'Kubernetes'],
      category: 'Web',
  ...
  console.log(`✅ Blog posts created: ${post1.title}, ${post2.title}`);
  ...
  await prisma.contactSubmission.create({
    data: {
      ...
      message: 'Hi Edward, I am interested in discussing a potential cloud architecture project. Would you be available for a call next week?',
      read: false,
    },
  });
  ...
  await prisma.consultationSubmission.create({
    data: {
      ...
      timeline: '3-6 months',
      description: 'We need help migrating our legacy monolith to a microservices architecture on AWS. Current stack is Java/Spring Boot with MySQL.',
      read: false,
    },
  });
